...
class TestDependencyOrdering(unittest.TestCase):
    ...
    def test_topological_sort_linear_dependencies(self):
        ...
        self.db_manager.add_story_relationship(story_a.id, story_b.id, "depends_on")
        self.db_manager.add_story_relationship(story_b.id, story_c.id, "depends_on")

        # Get topological order
        ordered_stories = self.db_manager.get_stories_topological_order([story_a.id, story_b.id, story_c.id])
    ...
    def test_topological_sort_parallel_dependencies(self):
        ...
        self.db_manager.add_story_relationship(story_a.id, story_b.id, "depends_on")
        self.db_manager.add_story_relationship(story_a.id, story_c.id, "depends_on")
        ...
        self.assertIn(story_b.id, ordered_stories[:2])  # B should be in first two
        self.assertIn(story_c.id, ordered_stories[:2])  # C should be in first two
    ...
    def test_dependency_priority_calculation(self):
        ...
        self.db_manager.add_story_relationship(story_a.id, story_b.id, "depends_on")
        self.db_manager.add_story_relationship(story_b.id, story_c.id, "depends_on")
        self.db_manager.add_story_relationship(story_c.id, story_d.id, "depends_on")

        # Calculate priorities
        priorities = self.db_manager.calculate_dependency_priorities([story_a.id, story_b.id, story_c.id, story_d.id])

        # Base story should have highest priority (lowest number = highest priority)
    ...
    def test_dependency_cycle_resolution(self):
        ...
        with self.assertRaises(ValueError):
            self.db_manager.add_story_relationship(story_c.id, story_a.id, "depends_on")
    ...
    def test_get_ordered_stories_for_epic(self):
        ...
        self.db_manager.add_story_relationship(us1.id, us2.id, "depends_on")
        self.db_manager.add_story_relationship(us2.id, us3.id, "depends_on")

        # Get ordered stories for epic
        ordered_stories = self.db_manager.get_ordered_stories_for_parent(epic.id)
    ...
    def test_dependency_depth_analysis(self):
        ...
        # Create dependencies
        self.db_manager.add_story_relationship(top_story.id, mid_story.id, "depends_on")
        self.db_manager.add_story_relationship(mid_story.id, base_story.id, "depends_on")
        ...
        self.assertEqual(depths[top_story.id], 2)  # Depends on mid, which depends on base
    ...
    def test_dependency_visualization(self):
        ...
        visualization = self.db_manager.generate_dependency_visualization([story_a.id, story_b.id, story_c.id])
