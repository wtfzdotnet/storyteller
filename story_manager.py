...
class StoryOrchestrator:
    ...
    async def create_refactor_tickets(self, refactor_request: str, refactor_type: str,
                                     target_repositories: Optional[List[str]] = None,
                                     specific_files: Optional[List[str]] = None) -> Dict[str, Optional[UserStory]]:
        """
        ...
        results = {}
        logger.info(f"Creating refactor tickets across repositories: {target_repositories}")
        ...
        for repo_key in target_repositories:
            ...
            try:
                ...
                ticket = await self._create_repository_refactor_ticket(
                    refactor_request, refactor_type, repo_key, repo_config, relevant_files
                )
                results[repo_key] = ticket
                
                if ticket:
                    logger.info(f"Created refactor ticket #{ticket.id} in repository {repo_config.name}")
                else:
                    logger.error(f"Failed to create refactor ticket in repository {repo_config.name}")
                    
            except Exception as e:
                logger.error(f"Error creating refactor ticket in repository {repo_key}: {e}")
                results[repo_key] = None
        
        return results

    async def _create_single_refactor_ticket(self, refactor_request: str, refactor_type: str,
                                           specific_files: Optional[List[str]] = None) -> Optional[UserStory]:
    ...
    async def _create_repository_refactor_ticket(self, refactor_request: str, refactor_type: str,
                                               repo_key: str, repo_config, relevant_files: List[str]) -> Optional[UserStory]:
    ...
    async def _discover_relevant_files(self, refactor_request: str, refactor_type: str,
                                     repo_key: Optional[str], specific_files: Optional[List[str]]) -> List[str]:
        """
        ...
        try:
            ...
            if file_suggestions:
                ...
                logger.info(f"AI suggested {len(relevant_files)} relevant file patterns")
            else:
    ...
    def _get_default_file_patterns(self, refactor_type: str, repo_key: Optional[str]) -> List[str]:
        ...
        defaults = {
            ...
            "modernize": ["**/*.py", "**/*.js", "**/*.ts", "package.json", "requirements.txt"],
            "general": ["**/*.py", "**/*.js", "**/*.ts", "**/*.md"]
        }
        ...
        if repo_key:
            repo_config = self.config.multi_repository_config.get_repository(repo_key) if self.config.is_multi_repository_mode() else None
            if repo_config:
                if repo_config.type == "backend":
                    patterns = ["**/*.py", "**/*.sql", "**/*.yml", "**/*.json"]
                elif repo_config.type == "frontend":
                    patterns = ["**/*.js", "**/*.ts", "**/*.jsx", "**/*.tsx", "**/*.css", "**/*.scss"]
                elif repo_config.type == "mobile":
                    patterns = ["**/*.swift", "**/*.kt", "**/*.dart", "**/*.js", "**/*.ts"]
    ...
    def _create_refactor_prompt(self, refactor_request: str, refactor_type: str,
                              repo_key: Optional[str], relevant_files: List[str]) -> str:
...
"""
    ...
    async def _create_immediate_ticket(self, prompt: str, roles_to_consult: List[str],
                                     refactor_type: str, relevant_files: List[str],
                                     target_repository_key: Optional[str] = None) -> Optional[UserStory]:
        ...
        labels = ["refactor", f"refactor/{refactor_type}", "ready-for-development"] + role_labels
        ...
        try:
            ...
            if created_issue:
                ...
                if relevant_files:
                    ...
                    self.github_service.add_comment_to_issue(created_issue.number, files_comment)

                logger.info(f"Successfully created immediate refactor ticket #{created_issue.number}")
                return user_story
            else:
