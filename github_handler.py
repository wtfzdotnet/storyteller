...
import github
from github import GithubException, Issue, IssueComment, UnknownObjectException
...
class GitHubService:
    ...
    def _initialize_client(self):
        ...
        try:
            self._github_client = github.Github(
                self.token, timeout=self.timeout, retry=self.max_retries
            )
            ...
            error_msg = (
                f"Repository '{self.repository_name}' not found or access denied"
            )
            ...
            error_msg = (
                f"Failed to initialize GitHub client: {e.data.get('message', str(e))}"
            )
            logger.error(error_msg)
    ...
    def _handle_github_exception(
        self, e: GithubException, operation: str
    ) -> GitHubOperationResult:
        """Handle GitHub exceptions and return appropriate result."""
        error_message = (
            e.data.get("message", str(e)) if hasattr(e, "data") and e.data else str(e)
        )
    ...
    async def create_issue(
        ...
        def _create_issue():
            ...
            logger.info(
                f"Successfully created issue #{issue.number}: '{title}' in {self.repository_name}"
            )
            return issue
    ...
    async def get_issue(self, issue_number: int) -> GitHubOperationResult:
        ...
        def _get_issue():
            ...
            logger.debug(
                f"Successfully retrieved issue #{issue_number} from {self.repository_name}"
            )
            return issue
    ...
    def update_issue(
        ...
        if not issue:
            logger.error(
                f"Cannot update issue #{issue_number} as it was not found in {self.repository_name}."
            )
            return None
        ...
        if (
            labels is not None
        ):  # PyGithub handles label replacement or clearing with empty list
        ...
        if not edit_params:
            logger.info(
                f"No parameters provided for updating issue #{issue_number}. No changes made."
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully updated issue #{issue_number} in {self.repository_name}."
            )
            ...
            logger.error(
                f"Failed to update issue #{issue_number} in {self.repository_name}: {e.data.get('message', str(e))}"
            )
            return None
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while updating issue #{issue_number}: {e}"
            )
            return None
    ...
    async def add_comment_to_issue(
        ...
        def _add_comment():
            ...
            logger.info(
                f"Successfully added comment to issue #{issue_number} in {self.repository_name}"
            )
            return comment
    ...
    def get_issue_comments(self, issue_number: int) -> List[IssueComment]:
        ...
        if not issue:
            logger.error(
                f"Cannot get comments for issue #{issue_number} as it was not found."
            )
            return []
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(comments)} comments for issue #{issue_number} from {self.repository_name}"
            )
            return comments
        except GithubException as e:
            logger.error(
                f"Failed to retrieve comments for issue #{issue_number}: {e.data.get('message', str(e))}"
            )
            return []
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while retrieving comments for issue #{issue_number}: {e}"
            )
            return []
    ...
    def search_issues(self, query: str, qualifier: str = "") -> List[Issue]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(issues)} issues matching query '{query}' in {self.repository_name}"
            )
            return issues
        except GithubException as e:
            logger.error(
                f"Failed to search issues in {self.repository_name} with query '{query}': {e.data.get('message', str(e))}"
            )
            return []
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while searching issues with query '{query}': {e}"
            )
            return []
    ...
    def get_repository_labels(self) -> List[github.Label.Label]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(labels)} labels for repository {self.repository_name}"
            )
            return labels
        except GithubException as e:
            logger.error(
                f"Failed to retrieve labels for repository {self.repository_name}: {e.data.get('message', str(e))}"
            )
            return []
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while retrieving labels for {self.repository_name}: {e}"
            )
            return []

    def create_label(
        self, name: str, color: str, description: str = ""
    ) -> Optional[github.Label.Label]:
        """
        ...
        try:
            ...
            logger.info(
                f"Successfully created label '{name}' with color '#{color}' in {self.repository_name}"
            )
            return label
        except GithubException as e:
            # Check if error is because label already exists
            if e.status == 422 and any(
                "already_exists" in error.get("code", "")
                for error in e.data.get("errors", [])
            ):
                logger.warning(
                    f"Label '{name}' already exists in {self.repository_name}. Cannot create."
                )
            ...
            logger.error(
                f"Failed to create label '{name}' in {self.repository_name}: {e.data.get('message', str(e))}"
            )
            return None
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while creating label '{name}': {e}"
            )
            return None
    ...
    def update_label(
        ...
        try:
            ...
            final_color = (
                ...
                else color if color is not None else label.color
            )
            final_description = (
            ...
            if (
                ...
                or final_color != label.color
                or final_description != label.description
            ):

                label.edit(
                    name=final_name, color=final_color, description=final_description
                )
            else:
                logger.info(
                    f"No changes needed for label '{current_name}'. Current values match requested values."
                )
                return label
            logger.info(
                f"Successfully updated label '{current_name}' (new name: '{label.name}') in {self.repository_name}"
            )
            return label
        except UnknownObjectException:
            logger.error(
                f"Label '{current_name}' not found in {self.repository_name}. Cannot update."
            )
            return None
        except GithubException as e:
            logger.error(
                f"Failed to update label '{current_name}' in {self.repository_name}: {e.data.get('message', str(e))}"
            )
            return None
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while updating label '{current_name}': {e}"
            )
            return None

    async def add_label_to_issue(
        self, issue_number: int, label_name: str
    ) -> GitHubOperationResult:
        """
        ...
        def _add_label():
            ...
            if label_name.strip() in current_label_names:
                logger.debug(
                    f"Label '{label_name}' already present on issue #{issue_number}"
                )
                return True  # Already applied

            issue.add_to_labels(label_name.strip())
            logger.info(
                f"Successfully added label '{label_name}' to issue #{issue_number}"
            )
            return True
    ...
    async def remove_label_from_issue(
        ...
        def _remove_label():
            ...
            logger.info(
                f"Successfully removed label '{label_name}' from issue #{issue_number}"
            )
            return True
    ...
    async def update_issue(
        ...
        def _update_issue():
            ...
            if not edit_params:
                logger.debug(
                    f"No parameters provided for updating issue #{issue_number}"
                )
                return issue
    ...
    async def get_issue_comments(self, issue_number: int) -> GitHubOperationResult:
        ...
        def _get_comments():
            ...
            logger.debug(
                f"Retrieved {len(comments)} comments for issue #{issue_number}"
            )
            return comments
    ...
    async def create_label(
        ...
        def _create_label():
            ...
            # Validate hex color
            if (
                not all(c in "0123456789ABCDEFabcdef" for c in clean_color)
                or len(clean_color) != 6
            ):
                raise ValueError(f"Invalid hex color: {color}")

            try:
                label = self.repo.create_label(
                    name=name.strip(), color=clean_color, description=description
                )
                logger.info(
                    f"Successfully created label '{name}' with color '#{clean_color}'"
                )
                return label
    ...
    async def update_label(
        ...
        if not current_name or not current_name.strip():
            return GitHubOperationResult.error_result(
                "Current label name cannot be empty"
            )
        ...
        def _update_label():
            ...
            if color and (
                not all(c in "0123456789ABCDEFabcdef" for c in final_color)
                or len(final_color) != 6
            ):
                raise ValueError(f"Invalid hex color: {color}")

            # Check if changes are needed
            if (
                final_name == current_name.strip()
                and final_color == label.color
                and final_description == label.description
            ):
            ...
            logger.info(
                f"Successfully updated label '{current_name}' -> '{final_name}'"
            )
            return label
    ...
    async def batch_operations(self):
        ...
        try:
            ...
            if operations:
                logger.info(
                    f"Completed batch operation with {len(operations)} operations"
                )
    ...
    async def get_copilot_instructions(self, repository_name: str) -> Optional[str]:
        ...
        try:
            ...
            if file_content:
                ...
                logger.info(
                    f"Successfully retrieved copilot instructions from {repository_name}"
                )
                return content
            else:
            ...
            logger.warning(
                f"Failed to retrieve copilot instructions from {repository_name}: {e}"
            )
            return None
        except Exception as e:
            logger.error(
                f"Unexpected error retrieving copilot instructions from {repository_name}: {e}"
            )
            return None
...
def add_comment_to_issue(
    issue_number: int, comment_body: str
        ...
        async with gh_service.batch_operations() as ops:
            ...
            if issue_result.success:
                ...
                comment_result = await gh_service.add_comment_to_issue(
                    issue.number, "Test comment from enhanced service"
                )
                ...
                close_result = await gh_service.update_issue(
                    issue.number, state="closed", body="Test completed - closing issue"
                )
