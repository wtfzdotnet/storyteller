...
class GitHubStorageManager:
    ...
    def __init__(
        ...
        if (
            self.storage_config.cache_enabled
            and self.storage_config.deployment_context == "mcp"
        ):
            try:
    ...
    async def save_epic(
        self, epic: Epic, repository_name: Optional[str] = None
    ) -> Issue:
    ...
    async def get_epic(
        self, epic_id: str, repository_name: Optional[str] = None
    ) -> Optional[Epic]:
    ...
    async def _parse_epic_from_issue(self, issue: Issue) -> Optional[Epic]:
        ...
        try:
            ...
            frontmatter, content = self.frontmatter_parser.extract_frontmatter(
                issue.body or ""
            )
            ...
            epic = Epic(
                ...
                created_at=datetime.fromisoformat(
                    frontmatter.get("created_at", issue.created_at.isoformat())
                ),
                updated_at=datetime.fromisoformat(
                    frontmatter.get("updated_at", issue.updated_at.isoformat())
                ),
    ...
    async def save_user_story(
        self, user_story: UserStory, repository_name: Optional[str] = None
    ) -> Issue:
    ...
    async def save_sub_story(
        self, sub_story: SubStory, repository_name: Optional[str] = None
    ) -> Issue:
    ...
    async def save_expert_analysis(
        self,
        issue_number: int,
        ...
        await self.github_handler.add_issue_comment(
            repository_name or self.config.github_repository, issue_number, comment_body
        )

        logger.info(
            f"Saved expert analysis from {analysis.role_name} to issue #{issue_number}"
        )
    ...
    def _format_expert_analysis_comment(self, analysis: StoryAnalysis) -> str:
        ...
        if analysis.metadata:
            comment_parts.extend(["### Additional Information", ""])
    ...
    async def reconstruct_story_hierarchy(
        self, epic_issue_number: int, repository_name: Optional[str] = None
    ) -> Optional[StoryHierarchy]:
        ...
        try:
            ...
            epic_issue = await self.github_handler.get_issue(
            ...
            for user_story in user_stories:
                user_story_subs = await self._find_sub_stories_for_user_story(
                    user_story.id, repo_name
                )
                if user_story_subs:
            ...
            logger.info(
                f"Reconstructed hierarchy for Epic {epic.id} with {len(user_stories)} user stories"
            )
            return hierarchy
    ...
    async def _find_user_stories_for_epic(
        self, epic_id: str, repository_name: str
    ) -> List[UserStory]:
    ...
    async def _find_sub_stories_for_user_story(
        self, user_story_id: str, repository_name: str
    ) -> List[SubStory]:
        ...
        try:
            ...
            logger.error(
                f"Failed to find sub stories for user story {user_story_id}: {e}"
            )
            return []
    ...
    async def _parse_user_story_from_issue(self, issue: Issue) -> Optional[UserStory]:
        ...
        try:
            ...
            user_story = UserStory(
                ...
                title=self._extract_title_from_content(content)
                or issue.title.replace("User Story: ", ""),
                description=self._extract_description_from_content(content),
                status=StoryStatus(frontmatter.get("status", "draft")),
