...
async def process_story_state(
    ...
    try:
        ...
        if trigger_event == "issues" and action == "labeled":
            ...
            logger.info(f"[{issue_number}] No action needed for trigger: {trigger_event}")
...
async def _ensure_required_labels(
    ...
    if "story" in current_labels:
        state_labels = ["ready-for-development", "in-development", "completed", "blocked"]
        has_state = any(label in current_labels for label in state_labels)
        
        if not has_state:
            logger.info(f"[{issue_number}] Adding default state label: ready-for-development")
            await github_service.add_label_to_issue(issue_number, "ready-for-development")
