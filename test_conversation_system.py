...
async def test_conversation_models():
    ...
    print(f"✓ Backend messages: {len(backend_messages)}, Frontend messages: {len(frontend_messages)}")
    
    # Test conversation summary
    summary = conversation.get_conversation_summary()
    print(f"✓ Conversation summary: {summary['message_count']} messages, {summary['participant_count']} participants")
    ...
    try:
        ...
        conversation = await manager.create_conversation(
            ...
            initial_participants=[
                ...
                {"name": "Lead Developer", "role": "lead-developer", "repository": "backend"}
            ]
        )
        ...
        print(f"✓ Generated insights: {len(insights['key_decisions'])} decisions, {len(insights['repository_impacts'])} repositories")
        
        # List conversations
        all_conversations = manager.list_conversations()
        frontend_conversations = manager.list_conversations(repository="frontend")
        print(f"✓ Listed conversations: {len(all_conversations)} total, {len(frontend_conversations)} for frontend")
...
async def test_mcp_conversation_endpoints():
    ...
    try:
        ...
        server.conversation_manager.context_reader.get_repository_context = AsyncMock(return_value=None)
        ...
        create_request = MCPRequest(
            ...
            params={
                ...
                "participants": [
                    ...
                    {"name": "Backend Lead", "role": "lead-developer", "repository": "backend"}
                ]
            }
        )
        ...
        assert response.result["success"]
        print(f"✓ Retrieved conversation via MCP: {response.result['conversation']['title']}")
        ...
        assert response.result["success"]
        print(f"✓ Got conversation history via MCP: {len(response.result['history']['messages'])} messages")
