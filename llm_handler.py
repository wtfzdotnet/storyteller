...
class OpenAILLMConnector(BaseLLMConnector):
    ...
    async def get_response(self, prompt: str, model: str = None) -> str:
        ...
        try:
            logging.info(f"Sending prompt to OpenAI model {current_model}: '{prompt[:50]}...'")
...
class OllamaLLMConnector(BaseLLMConnector):
    def __init__(self, host_url: str, default_model: str = "llama3"):
        ...
        self.default_model = default_model
        logging.info(f"OllamaLLMConnector initialized with host: {host_url}, model: {self.default_model}")

    async def get_response(self, prompt: str, model: str = None) -> str:
        current_model = model if model else self.default_model
        try:
            logging.info(f"Sending prompt to Ollama model {current_model} at {self.client._host}: '{prompt[:50]}...'")
            ...
            logging.error(f"Ollama API response error (model: {current_model}, host: {self.client._host}): {e.error}")
            if "model not found" in e.error.lower(): # Check if this is how model not found is reported
                 logging.error(f"Ensure the model '{current_model}' is pulled and available on the Ollama server at {self.client._host}.")
            ...
            logging.error(f"An unexpected error occurred with Ollama (model: {current_model}, host: {self.client._host}): {e}")
            raise

class GitHubLLMConnector(BaseLLMConnector):
    def __init__(self, github_token: str, default_model: str = "gpt-4o-mini", github_repository: str = None):
        if not github_token:
        ...
        self.github_repository = github_repository or os.environ.get("GITHUB_REPOSITORY")
    ...
    async def get_response(self, prompt: str, model: str = None, repository_references: list = None) -> str:
        current_model = model if model else self.default_model
        ...
        if repository_references and self.github_repository:
            ...
            logging.info(f"Using repository-based prompt with references: {repository_context}")
        ...
        try:
            logging.info(f"Sending prompt to GitHub model {current_model}: '{prompt[:50]}...'")
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if not content:
                            logging.warning("GitHub models API response content is empty.")
                            return ""
                        logging.info(f"Received response from GitHub model {current_model}.")
                        return content.strip()
                    else:
                        error_text = await response.text()
                        logging.error(f"GitHub models API error {response.status}: {error_text}")
                        raise ValueError(f"GitHub models API error {response.status}: {error_text}")
...
class LLMService:
    def __init__(self, 
        ...
        logging.info(f"Initializing LLMService with effective provider: {self.provider_name}")

        if self.provider_name == LLMProvider.OPENAI.value:
            key = openai_api_key or OPENAI_API_KEY
            if not key:
                logging.error("OpenAI provider selected, but API key is not configured.")
            ...
            if not host: # Should be caught by default in config, but check again
                logging.error("Ollama provider selected, but host URL is not configured.")
            ...
            if not token:
                logging.error("GitHub provider selected, but GitHub token is not configured.")
                raise ValueError("GitHub token is required for the GitHub provider.")
            self.connector = GitHubLLMConnector(github_token=token, github_repository=self.github_repository)
            logging.info("Using GitHub LLM Connector.")
            if self.github_repository:
                logging.info(f"GitHub repository context enabled: {self.github_repository}")
        else:
            logging.error(f"Unsupported LLM provider: {self.provider_name}")
            raise ValueError(f"Unsupported LLM provider: {self.provider_name}. Choose from 'openai', 'ollama', or 'github'.")

    async def query_llm(self, prompt: str, model: str = None, repository_references: list = None) -> str:
        """
        ...
        if not self.connector:
            ...
            raise RuntimeError("LLMService was not properly initialized with a connector.")
        
        logging.debug(f"LLMService querying connector with prompt: '{prompt[:50]}...' and model: {model}")
        try:
            ...
            if self.provider_name == LLMProvider.GITHUB.value and repository_references:
                return await self.connector.get_response(prompt, model=model, repository_references=repository_references)
            else:
...
async def main_test():
    ...
    try:
        ...
        response_default = await llm_service_default.query_llm(f"Hello from the default provider! Tell me a fun fact about {DEFAULT_LLM_PROVIDER}.", model=test_model_default)
    ...
    if OPENAI_API_KEY:
        try:
            ...
            llm_service_openai = LLMService(provider_name="openai", openai_api_key=OPENAI_API_KEY)
            response_openai = await llm_service_openai.query_llm("Hello from OpenAI! What is 2+2?", model="gpt-3.5-turbo")
    ...
    if OLLAMA_API_HOST: # Assuming OLLAMA_API_HOST is set, e.g. to http://localhost:11434
        try:
            ...
            llm_service_ollama = LLMService(provider_name="ollama", ollama_host=OLLAMA_API_HOST)
            response_ollama = await llm_service_ollama.query_llm("Hello from Ollama! Why is the sky blue?", model="llama3") # or "mistral"
            print(f"Response from Ollama (llama3): {response_ollama}")
        except Exception as e:
            # More specific error message if Ollama server is not reachable
            if "Connection refused" in str(e) or (hasattr(e, 'error') and "dial tcp" in str(e.error)):
                print(f"Error testing Ollama provider: Could not connect to Ollama server at {OLLAMA_API_HOST}. Ensure Ollama is running.")
            elif "model not found" in str(e).lower():
                 print(f"Error testing Ollama provider: Model not found. Ensure you have pulled the model (e.g., `ollama pull llama3`). Details: {e}")
            else:
    ...
    if GITHUB_TOKEN:
        try:
            ...
            llm_service_github = LLMService(provider_name="github", github_token=GITHUB_TOKEN)
            response_github = await llm_service_github.query_llm("Hello from GitHub! What is the capital of France?", model="gpt-4o-mini")
