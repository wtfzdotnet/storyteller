...
class DatabaseManager:
    ...
    def get_stories_topological_order(self, story_ids: List[str]) -> List[str]:
        ...
        if len(result) != len(story_ids):
            raise ValueError("Circular dependency detected - cannot determine topological order")
            ...
            with self.get_connection() as conn:
                ...
                if dependencies:
                    ...
                    for dependency_id in dependencies:
                        if dependency_id in story_ids:  # Only consider dependencies in our set
    ...
    def generate_dependency_visualization(self, story_ids: List[str]) -> str:
        ...
        with self.get_connection() as conn:
            for story_id in story_ids:
                cursor = conn.execute(
                    "SELECT id, title, story_type FROM stories WHERE id = ?", (story_id,)
                )
                row = cursor.fetchone()
                if row:
                    stories[story_id] = {
        ...
        with self.get_connection() as conn:
            cursor = conn.execute(
                ...
                """.format(','.join('?' * len(story_ids)), ','.join('?' * len(story_ids))),
                story_ids + story_ids
            )
        ...
        try:
            ...
            for i, story_id in enumerate(topological_order, 1):
                ...
                lines.append(f"  {i:2d}. [{story['type']:10}] {story['title']} (Priority: {priority}, Depth: {depth})")
            ...
            for story_id in story_ids:
                ...
                if story_deps:
                    dep_titles = [stories.get(dep_id, {"title": "Unknown"})["title"] for dep_id in story_deps]
                    lines.append(f"  • {story['title']} → depends on → {', '.join(dep_titles)}")
                else:
