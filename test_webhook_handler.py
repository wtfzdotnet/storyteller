...
def test_webhook_signature_verification():
    ...
    handler = WebhookHandler(config)
    assert handler.verify_signature(payload, signature_header)  # Should pass when no secret
...
async def test_webhook_payload_processing():
    ...
    with patch.object(handler.database, 'get_stories_by_github_issue', return_value=[epic]):
