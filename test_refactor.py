...
class TestRefactorMode(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_config.multi_repository_config.get_repository.side_effect = (
            lambda key: {"backend": backend_config, "frontend": frontend_config}.get(
                key
            )
        )

        self.orchestrator = StoryOrchestrator(
            self.mock_llm_service, self.mock_github_service, self.mock_config
        )
    ...
    def test_create_refactor_prompt(self):
        ...
        self.assertIn(
            "Code is refactored according to the specified requirements", prompt
        )
