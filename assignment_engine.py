...
class AssignmentEngine:
    ...
    def _check_workload_constraints(self) -> AssignmentDecision:
        ...
        if current_workload.active_stories >= self.max_concurrent_assignments:
            return AssignmentDecision(
                ...
                explanation=(
                    f"Agent at capacity ({current_workload.active_stories}/"
                    f"{self.max_concurrent_assignments} active stories)"
                ),
    ...
    def _get_current_workload(self, assignee: str) -> WorkloadInfo:
        ...
        return WorkloadInfo(
            ...
            pending_stories=0,
            last_assignment=(
                datetime.now(timezone.utc) if self.assignment_history else None
            ),
        )
