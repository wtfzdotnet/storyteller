...
from role_analyzer import RoleAssignmentResult
...
class TestRoleAssignmentIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with (
            patch("story_manager.LLMHandler"),
    ...
    def test_assign_roles_with_manual_overrides(self):
        ...
        async def run_test():
            ...
            manual_assignments = [
                r for r in assignment_details.primary_roles if r.assigned_by == "manual"
            ]
    ...
    def test_assign_roles_context_failure_fallback(self):
        ...
        async def run_test():
            ...
            result = await self.processor.assign_roles_intelligently(
                story_content="Basic story content", target_repositories=["backend"]
            )
    ...
    def test_role_assignment_with_multiple_repositories(self):
        ...
        async def run_test():
            ...
            assignment_details = result["assignment_details"]
            self.assertEqual(
                set(assignment_details.assignment_metadata["repositories"]),
                {"backend", "frontend"},
            )
            self.assertEqual(
                set(assignment_details.assignment_metadata["repository_types"]),
                {"backend", "frontend"},
            )
