...
from multi_repo_context import (  # noqa: E402
    FileContext,
...
class TestContextAwareStoryGeneration(unittest.TestCase):
    ...
    def test_story_generation_includes_repository_context(self):
        ...
        with (
            patch.object(
                self.story_processor, "analyze_story_content"
            ) as mock_analyze_content,
            patch.object(
                self.story_processor.context_reader, "get_repository_context"
            ) as mock_get_repo_context,
            patch.object(
                self.story_processor.context_reader, "get_multi_repository_context"
            ) as mock_get_multi_context,
            patch.object(
                self.story_processor.llm_handler, "analyze_story_with_role"
            ) as mock_analyze,
            patch.object(
                self.story_processor.llm_handler, "synthesize_expert_analyses"
            ) as mock_synthesize,
        ):
            ...
            mock_get_repo_context.side_effect = [
                mock_backend_context,
                mock_frontend_context,
            ]
        ...
        with (
            ...
            async def run_test():
                ...
                backend_analysis = next(
                    (
                        a
                        for a in result.expert_analyses
                        if "fastapi" in a.analysis.lower()
                    ),
                    None,
                )
    ...
    def test_repository_specific_sub_story_creation(self):
        ...
        with (
            ...
            mock_synthesize.return_value = MagicMock(
                content=(
                    "Synthesized analysis with repository-specific technical details "
            ...
            async def run_test():
                ...
                backend_ctx = next(
                    ctx for ctx in repo_contexts if ctx["repo_type"] == "backend"
                )
                self.assertIn("python", backend_ctx.get("languages", {}))

                # Verify frontend context
                frontend_ctx = next(
                    ctx for ctx in repo_contexts if ctx["repo_type"] == "frontend"
                )
