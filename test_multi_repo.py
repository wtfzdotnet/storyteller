...
def test_config_loading():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            assert config.is_multi_repository_mode(), "Multi-repository mode should be enabled"
            assert len(config.get_repository_list()) == 2, "Should have 2 repositories"
            assert config.get_target_repository('backend') == 'test/backend', "Backend repository should be correct"
            assert config.get_target_repository('frontend') == 'test/frontend', "Frontend repository should be correct"
            assert config.get_repository_dependencies('frontend') == ['backend'], "Frontend should depend on backend"
            assert config.get_repository_dependencies('backend') == [], "Backend should have no dependencies"
...
def test_dependency_sorting():
    ...
    if get_config().is_multi_repository_mode():
        ...
        sorted_repos = orchestrator._sort_repositories_by_dependencies(['frontend', 'backend'])
        
        assert sorted_repos == ['backend', 'frontend'], f"Expected ['backend', 'frontend'], got {sorted_repos}"
...
def test_backward_compatibility():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            assert not test_config.is_multi_repository_mode(), "Multi-repository mode should be disabled"
            assert test_config.get_target_repository() == 'test/single', f"Should use single repository, got {test_config.get_target_repository()}"
            assert test_config.get_repository_list() == ['default'], "Should have default repository"
