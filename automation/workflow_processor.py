...
WORKFLOW_STATES = {
    ...
    "story/user-approved": {
        "next_states": [
            "story/ready"
        ],  # Goes back to ready for development after user approval
...
CLEANUP_RULES = {
    ...
    "story/finalized": [
        ...
        "consensus/*",
        "trigger/*",
    ],  # Keep some approval labels for transition
...
async def get_current_iteration_label(
    current_labels: List[str],
) -> Optional[str]:  # Made async
...
async def get_current_consensus_label(
    current_labels: List[str],
) -> Optional[str]:  # Made async
...
async def assign_labels_based_on_content(
    issue_number: int,
    issue_title: str,
    issue_body: str,
    ...
    logger.info(
        f"[{issue_number}] Analyzing content for initial labels (Title: {issue_title[:30]}...)."
    )
    suggested_labels = []
...
async def apply_label_cleanup_rules(
    issue_number: int,
    current_labels: List[str],
    new_state_label: Optional[str],
    github_service: GitHubService,
    ...
    logger.info(
        f"[{issue_number}] Applying label cleanup. Current: {current_labels}. New state: {new_state_label or 'N/A'}. Terminal: {is_terminal}"
    )
    ...
    if is_terminal and "_terminal_state_cleanup" in CLEANUP_RULES:
        logger.info(
            f"[{issue_number}] Adding terminal state cleanup rules for: {new_state_label}"
        )
    ...
    for pattern in labels_to_remove_patterns:
        if pattern.endswith("/*"):
            prefix = pattern[:-2]
            actual_labels_to_remove.update(
                l for l in current_labels if l.startswith(prefix)
            )
        elif pattern in current_labels:
            if pattern != new_state_label or (
                is_terminal and pattern in TERMINAL_STATES
            ):
    ...
    if actual_labels_to_remove:
        logger.info(
            f"[{issue_number}] Identified for removal: {list(actual_labels_to_remove)}"
        )
            ...
            if label_to_remove in effective_labels:
                ...
                try:
                    if github_service.remove_label_from_issue(
                        ...
                        logger.warning(
                            f"[{issue_number}] Failed to remove label '{label_to_remove}' (service reported failure)."
                        )
                except Exception as e:
                    logger.error(
                        f"[{issue_number}] Exception removing label '{label_to_remove}': {e}",
                        exc_info=True,
                    )
            else:
                logger.debug(
                    f"[{issue_number}] Label '{label_to_remove}' was in removal set but not in current effective labels. Skipping."
                )
    else:
        logger.info(
            f"[{issue_number}] No labels matched for cleanup based on state '{new_state_label or 'N/A'}' and terminal={is_terminal}."
        )
...
def get_consensus_label_from_score(score: int) -> str:
    ...
    if score > 100:
        score = 100
    if score <= 20:
        return "consensus/0-20"
    if score <= 40:
        return "consensus/21-40"
    if score <= 60:
        return "consensus/41-60"
    if score <= 80:
        return "consensus/61-80"
    return "consensus/81-100"


def get_score_from_consensus_label(consensus_label: Optional[str]) -> int:
    if not consensus_label or not consensus_label.startswith("consensus/"):
        return 0
    try:
...
async def _create_repository_tickets_for_consensus(
    issue_number: int,
    github_service: GitHubService,
    llm_service: LLMService,
    ...
    if not story_orchestrator.config.is_multi_repository_mode():
        logger.info(
            f"[{issue_number}] Multi-repository mode not enabled, skipping repository ticket creation"
        )
        return
    ...
    try:
        ...
        if not story_details:
            logger.error(
                f"[{issue_number}] Could not retrieve story details for ticket creation"
            )
            return
        ...
        for (
            repo_key,
            repo_config,
        ) in story_orchestrator.config.multi_repository_config.repositories.items():
            enhanced_prompt = base_prompt

            # Try to get copilot instructions for this repository
            try:
                copilot_instructions = await github_service.get_copilot_instructions(
                    repo_config.name
                )
                if copilot_instructions:
                    logger.info(
                        f"[{issue_number}] Enhancing story for {repo_key} with copilot instructions"
                    )
                    enhanced_prompt = (
                        f"{base_prompt}\n\n"
                    ...
                    logger.info(
                        f"[{issue_number}] No copilot instructions found for {repo_key}"
                    )
            except Exception as e:
                logger.warning(
                    f"[{issue_number}] Failed to retrieve copilot instructions for {repo_key}: {e}"
                )
        ...
        if created_stories:
            ...
            for repo_key, story in created_stories.items():
                if story and story.id:
                    repo_config = story_orchestrator.config.multi_repository_config.get_repository(
                        repo_key
                    )
            ...
            if ticket_links:
                ...
                logger.info(
                    f"[{issue_number}] Added comment linking to {len(ticket_links)} created repository tickets"
                )
            else:
                logger.warning(
                    f"[{issue_number}] No valid tickets were created despite success response"
                )
        else:
        ...
        logger.error(
            f"[{issue_number}] Error in repository ticket creation: {e}", exc_info=True
        )
...
async def execute_auto_actions(
    ...
    logger.info(
        f"[{issue_number}] Executing auto-actions for state '{state_label}': {auto_actions}"
    )
    ...
    for action in auto_actions:
        try:
            if action == "assign_initial_roles":
                ...
                try:
                    await story_orchestrator.gather_feedback_and_iterate(
                        issue_number, DEFAULT_ROLES_FOR_FEEDBACK
                    )
                    logger.info(
                        f"[{issue_number}] Initial AI feedback generated successfully"
                    )
                    ...
                    logger.error(
                        f"[{issue_number}] Error during create_initial_feedback: {e}",
                        exc_info=True,
                    )
                ...
                logger.info(
                    f"[{issue_number}] Executing: add_needs_user_approval_label"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"[{issue_number}] Successfully created repository tickets"
                    )
                except Exception as e:
                    logger.error(
                        f"[{issue_number}] Error creating repository tickets: {e}",
                        exc_info=True,
                    )
            ...
            logger.error(
                f"[{issue_number}] Error executing auto-action '{action}': {e}",
                exc_info=True,
            )
...
async def _transition_to_state(
    ...
    story_orchestrator: StoryOrchestrator,
    actor: str,
    is_terminal_transition: bool = False,
) -> List[str]:
    logger.info(
        f"[{issue_number}] Transitioning to state: {new_state_label}. Is terminal: {is_terminal_transition}"
    )
    ...
    current_labels = await apply_label_cleanup_rules(
        ...
        new_state_label,
        github_service,
        is_terminal=is_terminal_transition,
    )
    logger.info(
        f"[{issue_number}] Transition to {new_state_label} complete. Effective labels post-transition: {current_labels}"
    )

    # Execute any auto-actions defined for the new state
    auto_actions = WORKFLOW_STATES.get(new_state_label, {}).get("auto_actions", [])
    if auto_actions:
        current_labels = await execute_auto_actions(
            issue_number,
            new_state_label,
            auto_actions,
            github_service,
...
async def process_story_state(
    ...
    current_labels = [
        label.strip() for label in current_labels_str.split(",") if label.strip()
    ]
    ...
    if config.auto_consensus_enabled:
        logger.info(
            f"[{issue_number}] Auto-consensus enabled: threshold={config.auto_consensus_threshold}%, max_iterations={config.auto_consensus_max_iterations}"
        )

    initial_story_label = await get_current_story_state_label(
        current_labels
    )  # Save initial state for comparison
    current_story_label = initial_story_label
    logger.info(
        f"[{issue_number}] Initial story state: {current_story_label or 'None'}"
    )
    ...
    if trigger_event == "issues" and action == "labeled":
        ...
        potential_terminal_label = (
            added_label_from_event
            if added_label_from_event in TERMINAL_STATES
            else None
        )
        if (
            not potential_terminal_label
        ):  # Fallback to checking current_labels if specific added label isn't known
            newly_added_terminal_labels = [
                l
                for l in current_labels
                if l in TERMINAL_STATES and l not in initial_story_label.split(",")
                if initial_story_label
            ]  # Compare with initial state
            if not newly_added_terminal_labels and any(
                l in TERMINAL_STATES for l in current_labels
            ):  # Check if any current label is terminal
                # If already in a terminal state, but not the one we are processing, it's a change.
                current_terminal_label_on_issue = next(
                    (l for l in current_labels if l in TERMINAL_STATES), None
                )
                if (
                    current_terminal_label_on_issue
                    and current_story_label != current_terminal_label_on_issue
                ):
        ...
        if potential_terminal_label and current_story_label != potential_terminal_label:
            logger.info(
                f"[{issue_number}] Issue manually labeled to terminal state: {potential_terminal_label}. Processing transition."
            )
            ...
            logger.info(
                f"[{issue_number}] End of processing due to terminal state transition: {current_story_label}"
            )
            return  # Exit early
    ...
    if trigger_event == "issues" and action == "opened":
        if not current_story_label:  # Only if no story label exists
            ...
            current_labels = await _transition_to_state(
                ...
                story_orchestrator,
                actor,
                is_terminal_transition=(new_state in TERMINAL_STATES),
            )
            ...
            logger.info(
                f"[{issue_number}] Issue 'opened' but already has state {current_story_label}. No initial setup."
            )
    ...
    while (
        current_story_label
        and current_story_label not in processed_in_run
        and current_story_label not in TERMINAL_STATES
    ):
        processed_in_run.add(current_story_label)
        previous_story_label_for_loop = (
            current_story_label  # Store before potential change
        )
        ...
        if iteration_label:
            try:
                current_iteration_num = int(iteration_label.split("/")[1])
            except (ValueError, IndexError):
                logger.warning(f"Could not parse iter num from {iteration_label}")
        ...
        if current_story_label == "story/draft":
            ...
            if "trigger/consensus-check" in current_labels:
                logger.info(
                    f"[{issue_number}] Initial feedback generated, transitioning to story/enriching"
                )
                current_labels = await _transition_to_state(
                    issue_number,
                    current_labels,
                ...
                logger.info(
                    f"[{issue_number}] Waiting for initial feedback generation to complete"
                )
            ...
            if trigger_iterate_label in current_labels:
                logger.info(
                    f"[{issue_number}] '{trigger_iterate_label}' detected for 'story/enriching'."
                )
                ...
                if not iteration_label:
                    next_iteration_num = 1  # First iteration

                if next_iteration_num <= iteration_limit:
                    logger.info(
                        f"[{issue_number}] Processing iteration {next_iteration_num} for 'story/enriching' (limit: {iteration_limit})."
                    )
                    if (
                        iteration_label
                    ...
                    try:
                        ...
                        logger.info(
                            f"[{issue_number}] Iteration {next_iteration_num} processing complete."
                        )

                        # If auto-consensus is enabled, automatically trigger consensus check
                        if config.auto_consensus_enabled:
                            logger.info(
                                f"[{issue_number}] Auto-consensus enabled, transitioning to reviewing for consensus check."
                            )
                            current_labels = await _transition_to_state(
                                issue_number,
                                current_labels,
                            ...
                            logger.info(
                                f"[{issue_number}] Conceptual: Check if all roles provided input to move to 'story/reviewing'."
                            )
                    except Exception as e:
                        logger.error(
                            f"[{issue_number}] Error during gather_feedback_and_iterate for iteration {next_iteration_num}: {e}",
                            exc_info=True,
                        )
                else:
                    logger.warning(
                        f"[{issue_number}] Iteration limit ({iteration_limit}) reached. Transitioning to 'story/blocked'."
                    )
                    current_labels = await _transition_to_state(
                        issue_number,
                        current_labels,
                ...
                logger.info(
                    f"[{issue_number}] In 'story/enriching', no '{trigger_iterate_label}'. Waiting for conditions or manual trigger."
                )

        elif current_story_label == "story/reviewing":
            trigger_consensus_check_label = "trigger/consensus-check"
            if (
                trigger_consensus_check_label in current_labels or action == "labeled"
            ):  # Process if triggered or if just entered state
                logger.info(
                    f"[{issue_number}] Processing 'story/reviewing'. Trigger: '{trigger_consensus_check_label if trigger_consensus_check_label in current_labels else 'state_entry'}'."
                )
                review_config = WORKFLOW_STATES.get("story/reviewing", {})
                roles_for_consensus = review_config.get(
                    "required_roles_for_consensus", DEFAULT_ROLES_FOR_CONSENSUS_CHECK
                )

                agreed = await story_orchestrator.check_agreement(
                    issue_number, roles_for_consensus
                )
                consensus_score = 100 if agreed else 40
                logger.info(
                    f"[{issue_number}] Consensus check: Agreed: {agreed} (Score: {consensus_score}%)"
                )
                ...
                if config.auto_consensus_enabled:
                    ...
                    iteration_limit = WORKFLOW_STATES.get("story/enriching", {}).get(
                        "iteration_limit", 5
                    )

                logger.info(
                    f"[{issue_number}] Using consensus threshold: {pass_threshold}%, iteration limit: {iteration_limit}"
                )
                ...
                if iteration_label:
                    try:
                        ...
                        logger.warning(
                            f"Could not parse iter num from {iteration_label}"
                        )

                if consensus_score >= pass_threshold:
                    logger.info(
                        f"[{issue_number}] Consensus met ({consensus_score}% >= {pass_threshold}%). Transitioning to 'story/consensus'."
                    )
                    current_labels = await _transition_to_state(
                        issue_number,
                        current_labels,
                    ...
                    if current_iteration_num < iteration_limit:
                        if config.auto_consensus_enabled:
                            logger.info(
                                f"[{issue_number}] Auto-consensus: Consensus not met ({consensus_score}% < {pass_threshold}%). Iterations {current_iteration_num}/{iteration_limit}. Automatically transitioning back to 'story/enriching'."
                            )
                            current_labels = await _transition_to_state(
                                issue_number,
                                current_labels,
                            ...
                            logger.info(
                                f"[{issue_number}] Consensus not met ({consensus_score}% < {pass_threshold}%). Iterations {current_iteration_num}/{iteration_limit}. Transitioning to 'story/enriching'."
                            )
                            current_labels = await _transition_to_state(
                                issue_number,
                                current_labels,
                        ...
                        logger.warning(
                            f"[{issue_number}] Consensus not met ({consensus_score}% < {pass_threshold}%) and iteration limit reached ({current_iteration_num}/{iteration_limit}). Transitioning to 'story/blocked'."
                        )
                        current_labels = await _transition_to_state(
                            issue_number,
                            current_labels,
                ...
                logger.info(
                    f"[{issue_number}] In 'story/reviewing'. Waiting for '{trigger_consensus_check_label}' or other relevant event."
                )

        elif current_story_label == "story/consensus":
            logger.info(
                f"[{issue_number}] In 'story/consensus'. Transitioning to 'story/ready'."
            )
            current_labels = await _transition_to_state(
                issue_number,
                current_labels,
            ...
            logger.info(
                f"[{issue_number}] In 'story/blocked'. Manual intervention typically required."
            )
            if WORKFLOW_STATES.get("story/blocked", {}).get(
            ...
            logger.info(
                f"[{issue_number}] In 'story/ready'. Can be finalized manually or check for finalization triggers."
            )

            # Check if finalize-story command has been run (story/finalized label added)
            if "story/finalized" in current_labels:
                logger.info(
                    f"[{issue_number}] Story has been finalized. Transitioning to finalized state."
                )
                current_labels = await _transition_to_state(
                    issue_number,
                    current_labels,
                ...
                logger.info(
                    f"[{issue_number}] Story is ready. Awaiting finalization trigger or manual action."
                )
                break  # Exit loop - terminal state for active automation unless finalized

        elif current_story_label == "story/finalized":
            logger.info(
                f"[{issue_number}] In 'story/finalized'. Processing finalization workflow."
            )

            # Auto-add needs/user-approval label if not present
            if "needs/user-approval" not in current_labels:
                if await github_service.add_label_to_issue(
                    issue_number, "needs/user-approval"
                ):
            ...
            if "approved/user" in current_labels:
                logger.info(
                    f"[{issue_number}] User approval detected. Transitioning to user-approved state."
                )
                current_labels = await _transition_to_state(
                    issue_number,
                    current_labels,
            ...
            logger.info(
                f"[{issue_number}] Story approved and ready for development workflow."
            )
        ...
        if (
            not current_story_label
            or current_story_label == previous_story_label_for_loop
        ):  # If state didn't change or no state label
    ...
    if current_story_label in TERMINAL_STATES:
        logger.info(
            f"[{issue_number}] Confirmed in terminal state '{current_story_label}'. Conceptual final actions if any."
        )
        if current_story_label == "story/archived" and WORKFLOW_STATES[
            "story/archived"
        ].get("auto_actions"):
            logger.info(
                f"[{issue_number}] Conceptual auto_action for '{current_story_label}': {WORKFLOW_STATES['story/archived']['auto_actions']}"
            )
        elif current_story_label == "story/split" and WORKFLOW_STATES[
            "story/split"
        ].get("auto_actions"):
            logger.info(
                f"[{issue_number}] Conceptual auto_action for '{current_story_label}': {WORKFLOW_STATES['story/split']['auto_actions']}"
            )
            logger.info(
                f"[{issue_number}] Conceptual: Suggest creating new issues for split parts and linking."
            )

    logger.info(
        f"[{issue_number}] End of processing for event '{trigger_event}', action '{action}'. Final labels (approx): {current_labels}"
    )
...
async def main_async():
    ...
    parser = argparse.ArgumentParser(
        description="Process GitHub issue events for story lifecycle automation."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--actor",
        type=str,
        help="GitHub username of the actor who triggered the event.",
    )
    parser.add_argument(
        "--log-level",
        type=str,
    ...
    logger.info(
        f"Workflow Processor started. Args: Issue: {args.issue_number}, Event: {args.trigger_event}, "
    ...
    try:
        ...
        llm_service = LLMService()
        story_orchestrator = StoryOrchestrator(
            llm_service=llm_service, github_service=github_service
        )

        # Enable repository-based prompts if environment variable is set
        use_repository_prompts = os.environ.get(
            "USE_REPOSITORY_PROMPTS", ""
        ).lower() in ("true", "1", "yes")
        if use_repository_prompts:
            story_orchestrator.use_repository_prompts = True
            logger.info(
                "Repository-based prompts enabled via USE_REPOSITORY_PROMPTS environment variable."
            )

        logger.info("Services initialized.")
    except ValueError as ve:
        logger.error(
            f"Initialization failed (config error): {ve}. Check env variables.",
            exc_info=True,
        )
        return
    except Exception as e:
    ...
    try:
        ...
        logger.error(
            f"[{args.issue_number}] Unhandled error in process_story_state: {e}",
            exc_info=True,
        )

    logger.info(
        f"Workflow Processor finished. Issue: {args.issue_number}, Event: {args.trigger_event}, Action: {args.action}"
    )
