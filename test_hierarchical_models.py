...
from models import Epic, UserStory, SubStory, StoryStatus, StoryType, StoryHierarchy
...
class TestHierarchicalModels(unittest.TestCase):
    ...
    def test_story_hierarchy_progress(self):
        ...
        user_story_2 = UserStory(epic_id=epic.id, title="US 2", status=StoryStatus.IN_PROGRESS)
        
        # Create sub-stories 
        sub_story_1 = SubStory(user_story_id=user_story_1.id, title="SS 1", status=StoryStatus.DONE)
        sub_story_2 = SubStory(user_story_id=user_story_1.id, title="SS 2", status=StoryStatus.DONE)
        sub_story_3 = SubStory(user_story_id=user_story_2.id, title="SS 3", status=StoryStatus.IN_PROGRESS)
