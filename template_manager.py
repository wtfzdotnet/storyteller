...
class ContextAwareTemplateManager:
    ...
    def __init__(self, templates_dir: Optional[Path] = None):
        ...
        self.templates_dir = (
            templates_dir or Path(__file__).parent / ".storyteller" / "templates"
        )
    ...
    def render_user_story_template(
        ...
        **kwargs,
        ...
        context["acceptance_criteria"] = (
            self._generate_context_aware_acceptance_criteria(
    ...
    def _generate_context_aware_acceptance_criteria(
        ...
        if repository_context:
            ...
            if any(
                ...
                for db in ["postgres", "mysql", "mongo", "sqlite"]
            ):
            ...
            if any(
                api in dep.lower()
                for dep in dependencies
                for api in ["fastapi", "flask", "express", "django"]
            ):
                criteria.append(
                    "API endpoints follow REST conventions and return appropriate status codes"
                )
            ...
            if any(
                fe in dep.lower()
                for dep in dependencies
                for fe in ["react", "vue", "angular"]
            ):
                criteria.append(
                    "Component renders correctly and handles user interactions"
                )
        ...
        if cross_repo_insights and cross_repo_insights.get("integration_points"):
            for integration_point in cross_repo_insights["integration_points"]:
                criteria.append(
                    f"Integration with {integration_point} functions correctly"
                )
    ...
    def _generate_technical_requirements(
        self, repo_type: str, repository_context: Optional[Dict[str, Any]] = None
    ) -> List[str]:
        ...
        if repo_type == "backend":
            ...
            if repository_context:
                ...
                if any(
                    style in dep.lower()
                    for dep in dependencies
                    for style in ["styled", "emotion", "tailwind"]
                ):
