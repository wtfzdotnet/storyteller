...
def test_complete_webhook_integration():
    ...
    db.create_github_issue_link("story_sub001", "test/backend-repo", 123, "https://github.com/test/backend-repo/pull/123")
    ...
    print(f"Webhook response: {webhook_response.status_code} - {webhook_response.json()}")
    ...
    webhook_response = client.post("/webhooks/github", json=pr_review_payload, headers={"X-Hub-Signature-256": create_webhook_signature(pr_review_payload)})
    assert webhook_response.status_code == 200
    ...
    webhook_response = client.post("/webhooks/github", json=pr_merged_payload, headers={"X-Hub-Signature-256": create_webhook_signature(pr_merged_payload)})
    assert webhook_response.status_code == 200
    ...
    webhook_response = client.post("/webhooks/github", json=push_payload, headers={"X-Hub-Signature-256": create_webhook_signature(push_payload)})
    print(f"Push webhook response: {webhook_response.status_code} - {webhook_response.json()}")
    assert webhook_response.status_code == 200
